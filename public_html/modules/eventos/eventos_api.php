...
function user_can_manage() {
  ...
  return in_array($c, array('desarrollador', 'director', 'secretaria'), true);
}
...
if (!is_valid_pagekey($pageKey)) {
  json_out(false, array('error' => 'page_key invalido.'), 400);
}
...
if ($action === 'top4') {
  // Top4 publico: NO incluir inactivos
...
if ($action === 'listar') {
  ...
  // Filtro calendario: aplica a inicio_fecha; opcion incluir sin fecha
  if ($desde || $hasta) {
    $range = array();
    if ($desde && $hasta) {
...
if ($action === 'get') {
  ...
  $id = (int)req_str('id');
  if ($id <= 0) json_out(false, array('error' => 'ID invalido.'), 400);

  $isPriv = auth_check() && user_can_manage();
  $sql = "SELECT id, titulo, parrafo, inicio_fecha, inicio_hora, fin_fecha, fin_hora,
                 coordinador, ponente, estado, tags_csv, foto_evento
          FROM l2601_eventos
          WHERE id = ? AND page_key = ? " . ($isPriv ? "" : " AND estado <> 'inactivo' ") . "
          LIMIT 1";
  $st = $mysqli->prepare($sql);
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
  json_out(false, array('error' => 'Metodo no permitido.'), 405);
}
...
if ($action === 'save') {
  ...
  if (!csrf_validate($_POST['csrf'] ?? null)) {
    json_out(false, array('error' => 'CSRF invalido. Recarga la pagina e intenta otra vez.'), 400);
  }
  ...
  if ($titulo === '') {
    json_out(false, array('error' => 'El titulo es obligatorio.'), 400);
  }
  if (mb_strlen($titulo) > 200) {
    json_out(false, array('error' => 'El titulo supera 200 caracteres.'), 400);
  }
  if ($parrafo !== '' && mb_strlen($parrafo) > 2000) {
    json_out(false, array('error' => 'El parrafo supera 2000 caracteres.'), 400);
  }
  ...
  // Para mensajes de tamano permitido
  ...
  if ($hasFoto) {
    ...
    if ($e === UPLOAD_ERR_INI_SIZE || $e === UPLOAD_ERR_FORM_SIZE) {
      json_out(false, array('error' => "La imagen es demasiado grande. Limite del servidor: upload_max_filesize={$uploadMax}, post_max_size={$postMax}."), 400);
    }
    if ($e !== UPLOAD_ERR_OK) {
    ...
    if ($tmp === '' || !is_uploaded_file($tmp) || @getimagesize($tmp) === false) {
      json_out(false, array('error' => 'El archivo no parece ser una imagen valida.'), 400);
    }
  }
  ...
  try {
    ...
    $stU->close();
    // si fallo update y habiamos subido foto nueva, la borramos para no dejar basura
...
json_out(false, array('error' => 'Accion invalida.'), 400);