...
#newsView .news-view-media-wrap{
  position:relative;
}
#newsView .news-view-expand{
  position:absolute;
  right:10px;
  bottom:10px;
  z-index:2;
...
#newsImageViewerModal .news-iv-stage{
  position: relative;
  width:100%;
  height:72vh;
  background:#0f1115;
...
#newsImageViewerModal .news-iv-stage.is-dragging{
  cursor: grabbing;
}
#newsImageViewerModal .news-iv-img{
  position:absolute;
  left:0;
  top:0;
...
#newsImageViewerModal .news-iv-toolbar .btn{
  min-width: 44px;
}
</style>
    ...
    <div class="modal-content" id="newsView">
      ...
      <div class="modal-body">
        <div class="row g-4 align-items-start">
          ...
          <div class="col-lg-4">
            <div class="news-view-media-wrap">
              ...
              <button type="button" class="btn news-view-expand" id="newsViewExpand" style="display:none;">Expandir</button>
            </div>
...
<!-- ===== MODAL VISOR DE IMAGEN (NOTICIAS) ===== -->
<div class="modal fade" id="newsImageViewerModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      ...
      <div class="modal-body pt-2">
        <div class="d-flex justify-content-end gap-2 mb-2 news-iv-toolbar">
          <button type="button" class="btn btn-outline-secondary btn-sm" id="newsIvZoomOut">-</button>
          <button type="button" class="btn btn-outline-secondary btn-sm" id="newsIvReset">100%</button>
          <button type="button" class="btn btn-outline-secondary btn-sm" id="newsIvZoomIn">+</button>
        </div>
        <div class="news-iv-stage" id="newsIvStage">
          <img id="newsIvImg" class="news-iv-img" alt="Imagen noticia">
        </div>
      </div>
    </div>
  </div>
...
(function(){
  ...
  function openView(it){
    ...
    var imgExpand = document.getElementById('newsViewExpand');
    ...
    if (img && imgPh) {
      if (it.imagen_portada) {
        currentViewImg = String(it.imagen_portada);
...
if (!j || !j.ok) {
  ...
  var ivModalEl = document.getElementById('newsImageViewerModal');
  var ivStage = document.getElementById('newsIvStage');
  var ivImg = document.getElementById('newsIvImg');
  var ivZoomIn = document.getElementById('newsIvZoomIn');
  var ivZoomOut = document.getElementById('newsIvZoomOut');
  var ivResetBtn = document.getElementById('newsIvReset');
  var ivScale = 1;
  var ivX = 0;
  var ivY = 0;
...
if (!j || !j.ok) {
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    var viewExpandBtn = document.getElementById('newsViewExpand');
    if (viewExpandBtn) {
