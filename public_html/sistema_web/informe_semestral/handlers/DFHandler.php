...
class DFHandler {
  ...
  /**
   * Guarda el "Visto Bueno" en Decanato de Facultad (DF).
  ...
  public function guardar(int $id_py, int $id_respuesta, string $accion, array $val, array $usr): array {
    if ($accion !== 'vb') return ['ok'=>false,'error'=>'Acción no soportada en DF'];
    ...
    try{
      ...
      // Llamada con 6 parámetros: tipo, estado, obs=NULL, dias=0
      ...
      // Actualiza estado de instancia en DF
      ...
      // Avance de flujo si fue aprobado
      ...
      // Notificaciones (derivación o aprobación total)
      if ($inst_id) {
        if ($nextId) {
