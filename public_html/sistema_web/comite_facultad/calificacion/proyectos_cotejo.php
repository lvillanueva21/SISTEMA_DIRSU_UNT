...
$_SESSION['oficina_actual'] = 'pcf'; // para usuarios del Comité de Facultad
...
<?php endif; ?>
  ...
  <?php endforeach; ?>
        ...
        <?php endif; ?>
<!-- 🔵 SECCIÓN LISTAS DE COTEJO -->
<div class="row justify-content-center align-items-center">
  <div class="col-12 mb-1 text-center">
    <label><b>Formatos Lista de Cotejo:</b></label>
  </div>

  <div class="col-auto mb-1 pr-1">
    <a href="https://docs.google.com/document/d/1GtM-AfDcfRfUtTV9uhUi7RJFUtt26Y2h/edit?tab=t.0"
       target="_blank"
  ...
  <div class="col-auto mb-1">
    <a href="https://docs.google.com/document/d/15RQPqVJTUDukv6mVG_lGnSf1Hx1Xdnd2/edit?tab=t.0"
       target="_blank"
...
$query = mysqli_query($conexion,
 "SELECT p.id, p.p2, p.p1, p.coordinador, p.fecha_inicio, p.fecha_fin,
  ...
  LEFT JOIN evaluaciones ev 
        ...
        AND ev.oficina = 'pcf' AND ev.tipo = 'cotejo'
  WHERE $condiciones
    AND up.activo = 1
  ORDER BY p.id ASC
  LIMIT $inicio, $limite");
  ...
  <div class="modal-dialog modal-xl" role="document">
    <div class="modal-content border-info">
      <div class="modal-header bg-primary text-white">
        <h5 class="modal-title"><i class="fas fa-info-circle"></i> Formulación y presentación de Proyecto</h5>
...
<div class="modal-header text-white" style="background-color: #3d9970;">
        <h5 class="modal-title"><i class="fas fa-calendar-alt"></i> Evaluación e informe Semestral</h5>
        <button type="button" class="close text-white" data-dismiss="modal">&times;</button>
      </div>
...
$(document).ready(function() {
  ...
  $.get('calificacion/presentacion.php', { id_py: id }, function(data) {
  $('#contenidoProyecto').html(data);

  // ✅ Cargar archivos subidos manualmente
  ...
  if (contenedor) {
    fetch('calificacion/gestion_archivos.php?id_py=' + id)
      ...
      .then(data => {
        ...
        for (const [clave, archivos] of Object.entries(data)) {
          const idsNavegacion = {
...
seccion.id = idsNavegacion[clave] || ""; // ← aquí asigna el id que la barra lateral espera
...
const titulo = `<strong>${nombres[clave]}</strong><br>`;
  ...
  const iconClass = isPDF ? 'fas fa-file-pdf text-danger' :
                   isExcel ? 'fas fa-file-excel text-success' :
                   'fas fa-file-alt text-secondary';

  const claseBoton = isPDF ? 'btn-outline-danger' : isExcel ? 'btn-outline-success' : 'btn-outline-secondary';
  const url = `calificacion/descarga_archivos.php?categoria=${clave}&id_py=${id}&archivo=${encodeURIComponent(nombre)}${isPDF ? '&ver=1' : ''}`;
  ...
  return `
    <div class="archivo-card d-flex align-items-center justify-content-between p-3 mb-2 border rounded shadow-sm bg-white">
      <div class="d-flex align-items-center" style="gap: 10px;">
        ...
        <div title="${nombre}" style="max-width: 300px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
          ${nombre}
        </div>
      </div>
...
$(document).on('click', '.btn-modal-semestral', function () {
  ...
  $('#tituloSemestral').text(titulo);
  $('#contenidoSemestral').html('<p class="text-center text-muted">Cargando informe semestral...</p>');
  $('#modalSemestral').modal('show');

  $.get('calificacion/semestral.php', { id_py: id }, function(data) {
    $('#contenidoSemestral').html(data);
  });
});
