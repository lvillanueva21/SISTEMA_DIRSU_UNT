...
if (count($proyectos_filtrados) > 0) {
    ...
    foreach ($proyectos_filtrados as $proyecto) {
        ...
        $pcf_rub_btn = '<span class="badge bg-secondary">No definido</span>';
        $id_py = $proyecto['id'];

        // Consultar pcf_rub
        $pcf_rub = null;
        $q1 = "SELECT pcf_rub FROM rutas_semestrales WHERE id_py = ?";
        if ($stmt1 = mysqli_prepare($conexion, $q1)) {
            mysqli_stmt_bind_param($stmt1, "i", $id_py);
            mysqli_stmt_execute($stmt1);
            mysqli_stmt_bind_result($stmt1, $pcf_rub);
        ...
        if ($pcf_rub !== null) {
            if ($pcf_rub == 0) {
                $pcf_rub_btn = '<span class="badge bg-primary">En Espera</span>';
            } elseif ($pcf_rub == 1) {
                $pcf_rub_btn = '<span class="badge bg-success">Aprobado</span>';
            } elseif ($pcf_rub == 2) {
                $pcf_rub_btn = '<span class="badge bg-danger">Observado</span>';
                ...
                if ($stmt_limite = mysqli_prepare($conexion, $q_limite)) {
                    ...
                    if (mysqli_stmt_fetch($stmt_limite) && !empty($pcf_limite)) {
                        ...
                        $pcf_rub_btn .= "<br>⏰Fecha límite:<br><b>{$fecha_format}</b>";
                    }
    ...
    echo $pagina_actual > 1
        ? '<li class="page-item"><a class="page-link" href="rubrica.php?pagina=' . ($pagina_actual - 1) . '">&laquo; Anterior</a></li>'
        : '<li class="page-item disabled"><span class="page-link">&laquo; Anterior</span></li>';

    for ($i = 1; $i <= $total_paginas; $i++) {
        $active = ($i == $pagina_actual) ? 'active' : '';
        echo '<li class="page-item ' . $active . '"><a class="page-link" href="rubrica.php?pagina=' . $i . '">' . $i . '</a></li>';
    }

    echo $pagina_actual < $total_paginas
        ? '<li class="page-item"><a class="page-link" href="rubrica.php?pagina=' . ($pagina_actual + 1) . '">Siguiente &raquo;</a></li>'
