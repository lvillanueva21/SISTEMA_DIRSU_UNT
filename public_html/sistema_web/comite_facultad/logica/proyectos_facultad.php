...
// Filtrar por la facultad del usuario tipo FACULTAD utilizando el dato de sesión $id_escuela
...
if ($keyword != '' && $estado_filtro != '') {
    // Orden de parámetros: $id_escuela (i), keyword (s), keyword (s), estado (i)
    mysqli_stmt_bind_param($stmt_usuarios, 'issi', $id_escuela, $keyword, $keyword, $estado_filtro);
} elseif ($keyword != '' && $estado_filtro == '') {
    mysqli_stmt_bind_param($stmt_usuarios, 'iss', $id_escuela, $keyword, $keyword);
} elseif ($keyword == '' && $estado_filtro != '') {
    mysqli_stmt_bind_param($stmt_usuarios, 'ii', $id_escuela, $estado_filtro);
} else {
    mysqli_stmt_bind_param($stmt_usuarios, 'i', $id_escuela);
}
