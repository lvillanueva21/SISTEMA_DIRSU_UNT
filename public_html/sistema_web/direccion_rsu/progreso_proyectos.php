...
<form method="GET" action="progreso_proyectos.php" style="margin: 10px 0;">
  <div class="row mx-3">
      ...
      <select class="form-control" name="facultad" id="facultad">
        ...
        <?php
          foreach ($facultades as $id => $nombre) {
              $selected = (isset($_GET['facultad']) && $_GET['facultad'] == $id) ? 'selected' : '';
    ...
    <div class="col-md-2">
      ...
      <select class="form-control" name="ods" id="ods">
        <option value="">Todos</option>
        <?php
          foreach ($ods as $id => $descripcion) {
              $selected = (isset($_GET['ods']) && $_GET['ods'] == $id) ? 'selected' : '';
