...
$oficina_actual = 'rsu';  // Oficina de la Dirección de RSU
...
$condiciones .= " AND up.activo = 1";
$condiciones .= " AND pp.id_periodo = $periodo_seleccionado";
  ...
  <?php endforeach; ?>
        ...
        <div class="col-md-4 mb-2">
          ...
          <select name="estado" class="form-control">
            ...
            <option value="0" <?= (isset($_GET['estado']) && $_GET['estado'] === '0') ? 'selected' : '' ?>>En Proceso</option>
...
if ($filtro_facultad !== 0) {          // <-- ahora se aplica para cualquier rol
 ...
 "SELECT p.id, p.p2, p.p1, p.coordinador, p.fecha_inicio, p.fecha_fin,
  ...
  LEFT JOIN evaluaciones ev 
      ...
      AND ev.oficina = '$oficina_actual' AND ev.tipo = '$tipo_evaluacion'
  WHERE $condiciones
  ORDER BY p.id ASC
  LIMIT $inicio, $limite");
...
<th>Cotejo</th>
  ...
  <?php if (trim($r['p2']) !== ''): ?>
    ...
    <span class="badge badge-info ml-1" style="font-size: 0.7rem;">
      <?= htmlspecialchars($r['nombre_periodo'] ?? 'Sin período') ?>
    </span>
  <?php else: ?>
...
<td>
  ...
  <span style="
    ...
    color: #FFFFFF;
    font-weight: bold;
    font-size: 0.8rem; /* tamaño base más pequeño */
    padding: 6px 10px;
    overflow: hidden;
    border-radius: 12px;
  ...
  ">
    <img src="../dust/img/dirsu_logo_128_128.png" alt="Logo" style="
      height: 24px; /* LOGO MÁS PEQUEÑO */
      margin-right: 6px;
      filter: drop-shadow(1px 1px 2px rgba(0,0,0,0.6));
    ">
    <div style="display: flex; flex-direction: column; align-items: center; line-height: 1.1;">
<div style="font-size: 0.8rem;">Aprobación</div>
<div style="font-size: 0.9rem;">' . htmlspecialchars($r['nombre_periodo']) . '</div>
    </div>
    <span style="
      position: absolute;
      top: 0;
...
<!-- Modal PROYECTO (estilo GRUPO 01) -->
    ...
    <div class="modal-content border-info">
      <div class="modal-header bg-primary text-white py-2">
        <h5 class="modal-title mb-0 d-flex align-items-center">
          <i class="fas fa-info-circle mr-2"></i> Formulación y presentación de Proyecto
        </h5>
        <div class="d-flex align-items-center">
          <button type="button"
                  class="btn btn-danger btn-sm mr-2 btn-print"
                  data-target="#contenidoProyecto"
                  data-title="Proyecto - Reporte">
      ...
      <div class="modal-body p-0">
        <div id="contenidoProyecto" style="height:78vh; overflow:auto;">
...
<!-- Modal SEMESTRAL (estilo GRUPO 01) -->
<div class="modal fade" id="modalSemestral" tabindex="-1" role="dialog">
  <div class="modal-dialog modal-xl modal-dialog-scrollable" role="document">
    <div class="modal-content border-success">
      <div class="modal-header text-white py-2" style="background-color:#3d9970;">
        <h5 class="modal-title mb-0 d-flex align-items-center">
      ...
      <div class="modal-body p-0">
        <div id="contenidoSemestral" style="height:78vh; overflow:auto;">
          <p class="text-center text-muted my-4">Cargando informe semestral...</p>
        </div>
      </div>
    </div>
  </div>
...
$(document).ready(function() {
  ...
  $.get('calificacion/cargar_cotejo.php', { id_py, id_periodo }, function(data) {
    if (data.success) {
...
$('#formCotejo').submit(function(e) {
  e.preventDefault();
  $.post('calificacion/guardar_cotejo.php', $(this).serialize(), function(resp) {
...
// Recargar solo la tabla con AJAX
...
<script>
/* ================== IMPRESIÓN A4 con tablas anchas en hoja horizontal ==================
   - Solo afecta al contenido impreso.
   - Tablas del sistema (Drive/archivos/ODS) NO se mueven.
   - Las tablas de usuario (en ítems tipo text/longtext/varchar) que desbordan se clonan a una
     página A4 landscape y la original se oculta SOLO en impresión.
======================================================================================== */

function rsu_imprimir_contenedor(selector, titulo = 'Reporte') {
  try {
    ...
    // Estilos: retrato por defecto + página nombrada "land" para landscape
    var styles = '\
      <style>\
        ...
        *{-webkit-print-color-adjust:exact; print-color-adjust:exact;}\
        ...
        /* Ocultar UI de modal (si quedara algún resto) */\
        .modal,.modal-dialog,.modal-content,.modal-header,.modal-footer{display:none !important;}\
        .btn, .badge, .close { display:none !important; }\
        /* Ocultar barra lateral de navegación para imprimir contenido limpio */\
        .navegacion-lateral { display:none !important; }\
        .contenedor-scroll .flex-grow-1 { width:100% !important; }\
        /* Evitar cortes feos en ítems */\
        .item-proyecto{ page-break-inside:avoid; }\
        /* Evitar íconos (no cargamos FontAwesome en el iframe) */\
        .fas, .far { display:none !important; }\
        /* Páginas landscape generadas por script */\
        .print-only{ display:none; }\
        @media print{\
          .print-only{ display:block !important; }\
          .print-hide{ display:none !important; }\
        }\
        .rsu-landscape-page{ page: land; page-break-before: always; }\
        .rsu-landscape-title{ font-weight:700; margin:0 0 8px 0; }\
        .rsu-landscape-table{ width:100% !important; table-layout:auto !important; }\
      </style>';
    ...
    doc.write('<!DOCTYPE html><html><head><meta charset="utf-8"><title>'
              ...
              + root.innerHTML
              + '<script>(' + (function promoteWideTables(){
                  try{
                    var body = document.body;

                    function isSystemTable(tb){
                      // drive/apple/archivos del sistema
                      if (tb.closest('.drive-table') || tb.closest('.apple-table')) return true;
                      if (tb.querySelector(".btn-file-pdf,.btn-file-excel,.btn-file-word,.btn-drive")) return true;
                      return false;
                    }
                    function isFromUserLongText(tb){
                      // Solo tablas dentro de items cuyo tipo es longtext/text/varchar
                      var sec = tb.closest("section.item-section");
                      if(!sec) return false;
                      var badge = sec.querySelector(".badge-type");
                      if(!badge) return false;
                      var t = (badge.textContent||"").trim().toLowerCase();
                      return t==="longtext" || t==="text" || t==="varchar";
                    }
                    function isWide(tb){
                      // Heurística: overflow horizontal o muchas columnas
                      var cols = 0;
                      if (tb.tHead && tb.tHead.rows.length) cols = tb.tHead.rows[0].cells.length;
                      else if (tb.rows.length) cols = tb.rows[0].cells.length;
                      // Fuerza layout antes de medir
                      var prevLayout = tb.style.tableLayout, prevWidth = tb.style.width, prevMaxW = tb.style.maxWidth;
                      tb.style.tableLayout = "auto"; tb.style.width = "auto"; tb.style.maxWidth = "none";
                      var cw = tb.clientWidth, sw = tb.scrollWidth;
                      tb.style.tableLayout = prevLayout; tb.style.width = prevWidth; tb.style.maxWidth = prevMaxW;
                      return (sw - cw > 2) || (cols >= 8);
                    }

                    var tables = Array.prototype.slice.call(body.querySelectorAll("table"));
                    tables.forEach(function(tb){
                      if (isSystemTable(tb)) return;
                      if (!isFromUserLongText(tb)) return;
                      if (!isWide(tb)) return;

                      // Clonar a página landscape y ocultar original SOLO en impresión
                      var wrapper = document.createElement("div");
                      wrapper.className = "rsu-landscape-page print-only";

                      var titleEl = tb.closest("section.item-section") && tb.closest("section.item-section").querySelector(".item-title");
                      var titleTxt = titleEl ? (titleEl.textContent||"").trim() : "Tabla";
                      var header = document.createElement("div");
                      header.className = "rsu-landscape-title";
                      header.textContent = titleTxt;
                      wrapper.appendChild(header);

                      var clone = tb.cloneNode(true);
                      clone.classList.add("rsu-landscape-table");
    ...
    // Espera breve para que el script de dentro marque/mueva tablas y luego imprime
    setTimeout(function(){
...
/* Delegado: imprime el contenedor indicado en data-target */
document.addEventListener('click', function(e){
  var btn = e.target.closest('.btn-print');
  if (!btn) return;
  var target = btn.getAttribute('data-target') || '';
  var titulo = btn.getAttribute('data-title') || (btn.textContent || '').trim() || 'Reporte';
  rsu_imprimir_contenedor(target, titulo);
});
</script>
<script>
(function () {
  // Proyecto
  document.addEventListener('click', function (e) {
    const a = e.target.closest('#contenidoProyecto a[href^="#"]');
    if (!a) return;
  ...
  // Semestral
  document.addEventListener('click', function (e) {
    const a = e.target.closest('#contenidoSemestral a[href^="#"]');
    if (!a) return;
    const id = a.getAttribute('href');
    if (!id || id === '#') return;
    const target = document.querySelector(id);
