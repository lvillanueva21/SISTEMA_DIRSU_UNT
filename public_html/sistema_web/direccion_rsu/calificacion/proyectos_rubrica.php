...
$query = mysqli_query($conexion,"
 ...
 LEFT JOIN evaluaciones ev ON ev.id_py = p.id
  ...
  AND ev.oficina = 'rsu' AND ev.tipo = 'rubrica'
 WHERE $condiciones
 AND up.activo = 1
 ORDER BY p.id ASC
 LIMIT $inicio,$limite");
...
if ($estado_int === 0) {
  ...
  ">
    ...
    <div style="display: flex; flex-direction: column; align-items: center; line-height: 1.1;">
      <div style="font-size: 0.8rem;">Aprobación</div> <!-- texto reducido -->
      <div style="font-size: 0.9rem;">2024-II</div>     <!-- ligeramente más grande -->
    </div>
    <span style="
      position: absolute;
      top: 0;
...
?>
  ...
  <button class="btn btn-warning btn-sm btn-calificar-rubrica"
        data-id="<?= $r['id'] ?>" data-periodo="<?= $periodo_seleccionado ?>"
