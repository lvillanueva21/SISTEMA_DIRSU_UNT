<?php
// presentacion/logica/guardar_item.php — VERSIÓN ANTERIOR (sin cambios por tema de video)
...
try {
    ...
    foreach ($items as $it) {
        ...
        if (isset($vals[$idIt]) && ri_val_esta_lleno($vals[$idIt], $tp)) $completos++;
    }
    ...
    if ($completos >= 1) { if ($nuevoEstado === 0) $nuevoEstado = 1; else $nuevoEstado = 2; }

    $up = $conexion->prepare("UPDATE sm_respuestas SET estado=?, actualizado_at=CURRENT_TIMESTAMP WHERE id=?");
    $up->bind_param("ii", $nuevoEstado, $id_respuesta);
    if (!$up->execute()) throw new RuntimeException("Error al actualizar cabecera: ".$up->error);
    $up->close();
...
flash('Ítem guardado correctamente.', 'success');
back_to_index($dest);
