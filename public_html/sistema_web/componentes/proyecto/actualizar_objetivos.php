...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $general = isset($_POST['general']) ? $_POST['general'] : null;
        $especificos = isset($_POST['especificos']) ? $_POST['especificos'] : null;
        $metas = isset($_POST['metas']) ? $_POST['metas'] : null;

        // Preparar la consulta para actualizar los datos en la tabla proyectos
        $sql_update_proyecto = "UPDATE proyectos 
                                SET general = ?, especificos = ?, metas = ?
                                WHERE id = ?";
        ...
        $stmt->bind_param('sssi', $general, $especificos, $metas, $id_proyecto);
