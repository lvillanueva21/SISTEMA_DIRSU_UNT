...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $pre_dis = isset($_POST['pre_dis']) ? $_POST['pre_dis'] : null;
        $pre_nodis = isset($_POST['pre_nodis']) ? $_POST['pre_nodis'] : null;

        // Preparar la consulta para actualizar los datos en la tabla proyectos
        $sql_update_proyecto = "UPDATE proyectos 
                                SET pre_dis = ?, pre_nodis = ?
                                WHERE id = ?";
        ...
        $stmt->bind_param('ssi', $pre_dis, $pre_nodis, $id_proyecto);
