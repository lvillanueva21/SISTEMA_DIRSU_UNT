...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $ser_dis = isset($_POST['ser_dis']) ? $_POST['ser_dis'] : null;
        $ser_nodis = isset($_POST['ser_nodis']) ? $_POST['ser_nodis'] : null;

        // Preparar la consulta para actualizar los datos en la tabla proyectos
        $sql_update_proyecto = "UPDATE proyectos 
                                SET ser_dis = ?, ser_nodis = ?
                                WHERE id = ?";
        ...
        $stmt->bind_param('ssi', $ser_dis, $ser_nodis, $id_proyecto);
