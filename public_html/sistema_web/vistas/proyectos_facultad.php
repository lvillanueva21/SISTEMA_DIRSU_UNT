<?php 
session_start();
$id_proyecto = $_SESSION['id_py']; // Ahora se filtra por id_py en lugar de id_depa
...
// Filtrar por el proyecto del usuario utilizando el dato de sesión $id_proyecto
$query_usuarios .= " AND u.id_py = ? ";
...
if ($keyword != '' && $estado_filtro != '') {
    // Orden de parámetros: $id_proyecto (i), keyword (s), keyword (s), estado (i)
    mysqli_stmt_bind_param($stmt_usuarios, 'issi', $id_proyecto, $keyword, $keyword, $estado_filtro);
} elseif ($keyword != '' && $estado_filtro == '') {
    mysqli_stmt_bind_param($stmt_usuarios, 'iss', $id_proyecto, $keyword, $keyword);
} elseif ($keyword == '' && $estado_filtro != '') {
    mysqli_stmt_bind_param($stmt_usuarios, 'ii', $id_proyecto, $estado_filtro);
} else {
...
if (mysqli_num_rows($result_usuarios) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result_usuarios)) {
        ...
        if ($row_proyecto = mysqli_fetch_assoc($result_proyecto)) {
            ...
            $id_proyecto_row = $row_proyecto['id'];
            ...
            $proyecto_info .= '<b>id_py: </b>' . htmlspecialchars($id_proyecto_row) . '<br>';
        ...
        if ($proyecto_registrado) {
            echo '<button class="btn btn-primary btn-sm" style="font-size: 10px;" onclick="verDetalle(' . $id_proyecto_row . ')"><i class="fas fa-info-circle"></i> Proyecto</button>';
            echo '<button class="btn btn-success btn-sm" style="font-size: 10px;" onclick="verSemestre(' . $id_proyecto_row . ')"><i class="fas fa-calendar"></i> Semestral</button>';
            echo '<button class="btn btn-warning btn-sm" style="font-size: 10px;" onclick="verEvaluacion(' . $id_proyecto_row . ')"><i class="fas fa-star"></i> Evaluación</button>';
        } else {
