<?php
// /sistema_web/evaluacion/control_oficinas.php
// Reglas de visibilidad de botones y habilitación por oficina/estado.
include_once __DIR__ . '/../componentes/db.php';

/** Oficina asociada a un rol → código corto de catálogo eva_oficinas */
function oficinaCodigoPorRol(int $id_rol): ?string {
    switch ($id_rol) {
        case 5: return 'PCF'; // Comité de Facultad
        case 4: return 'DD';  // Dirección de Departamento
        case 3: return 'DF';  // Decanato de Facultad
        case 1: return 'RSU'; // Dirección RSU
        // 0: Admin (sin oficina), 2: Coordinador (sin oficina)
        default: return null;
    }
}

/** Qué botones “calificar” puede VER cada rol */
...
function estadoEvaluacionActualPorProyecto(int $id_py): ?array {
    global $conexion;
    // Tomamos la evaluación más reciente del proyecto y su oficina actual por ID
    $sql = "SELECT
                e.id                AS eval_id,
...
function puedeClickearAccion(int $id_rol, string $accion, int $id_py): array {
    ...
    if ($id_rol !== 0) {
        ...
        if (!$ofRol || !$st['oficina_id'] || (int)$st['oficina_id'] !== (int)$ofRol['id']) {
            $out['why'] = 'Proyecto no está en su oficina actual'; 
            $out['state'] = $st;
            return $out;
        }
    }
    ...
    if ($inst !== 'en_espera') { $out['why'] = 'La oficina no está en espera de revisión'; $out['state'] = $st; return $out; }
