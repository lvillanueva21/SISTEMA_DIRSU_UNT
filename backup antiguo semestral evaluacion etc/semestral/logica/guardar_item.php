<?php
// presentacion/logica/guardar_item.php — SIN side-effects de estado (estado se maneja sólo por acciones explícitas)
...
// Bloqueo por estado (no se puede editar si está en revisión o aprobado)
if (in_array((int)$resp['estado'], [1, 2], true)) {
    $msg = ((int)$resp['estado'] === 1)
        ? 'No puedes guardar porque el informe está en revisión. Anula la solicitud para seguir editando.'
        : 'No puedes guardar porque el informe ya está aprobado.';
    flash($msg, 'warning');
    back_to_index();
}
...
try {
    ...
    // IMPORTANTE: ya NO actualizamos sm_respuestas.estado aquí.
    // Solo recalculamos para navegación posterior.
